...
def vote(request, slug):
    ...
    vote_value = data['vote'] # The user's new vote.
    ...
    if user.is_anonymous: # User not logged in. Can't vote.
        ...
        if JokeVote.objects.filter(user=user, joke=joke).exists():
            ...
            if joke_vote.vote == vote_value: # User's new vote is the same as old vote.
                msg = 'Right. You told us already. Geez.'
            else: # User changed vote.
                joke_vote.vote = vote_value # Update JokeVote instance.
                joke_vote.save() # Save.
                if vote_value == -1:
            ...
            joke_vote = JokeVote(user=user, joke=joke, vote=vote_value)
            if vote_value == -1:
    ...
    response = {
        'msg': msg,
        'likes': joke.num_likes,
        'dislikes': joke.num_dislikes,
    }
